...
def top_N_NEs(
    ...
    :param aliases: specify if also NEs aliases have to be retrieved or not
    """
...
def get_json_entity(entity_id, aliases=False):
    ...
    # compute props based on aliases requested or not
...
def top_N_NEs_strings(input_csv_ranking_file_path, top_N=1000, aliases=False):
    """
    It labels the specified number of NEs in the input file, and return the resulting string labels in a list
    :param input_csv_ranking_file_path: input csv file containing the NEs IDs. It has to be a table
                                     with a header, and first column NE ID in the format Qxxxx
    :param top_N: number of top lines to be processed in the input file (default: 1000)
    :param aliases: specify if also the aliases have to be retrieved (True) or not (False) [default: no]
    :return: the list of the NEs labels
    """
